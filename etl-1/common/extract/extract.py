...
import pandas as pd
import time
import etl.common.utils.get_timestamp as ts
import etl.common.extract.scrape_schedule_page as schedule
import etl.common.extract.scrape_game_page as game
import etl.common.extract.scrape_team_page as team
import etl.common.extract.get_geocode_data as geo
...
def instantiate_logfile(league: str):
    ...
    extract_logfile_path = f'./logs/{league}_extract_{timestamp}.log'
    extract_logfile = open(extract_logfile_path, 'a')
    return extract_logfile

def create_games_df(league: str, game_ids: list, extract_logfile: object):
...
def create_teams_df(league: str, team_ids: list, extract_logfile: object):
...
def full_extract(league: str, year: int, weeks: int):
    ...
    extract_logfile.write('\n~~ Retrieving Game Data ~~\n')
    games_raw = create_games_df(league, game_ids, extract_logfile)

    print('\n~~ Retrieving Schools Data ~~')
    extract_logfile.write('\n~~ Retrieving Schools Data ~~\n')
    teams_raw = create_teams_df(league, games_raw['away_school_id'].unique(), extract_logfile)

    print('\n~~ Retrieving Locations Data ~~')
    extract_logfile.write('\n~~ Retrieving Locations Data ~~\n')
    locations_raw = create_locations_df(games_raw['stadium'], games_raw['location'], extract_logfile)
